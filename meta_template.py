...
class MetaTemplate(nn.Module):
	...
	def parse_feature(self, x, is_feature):   # x=5*20*512*7*7
		x = Variable(x.cuda())

		if is_feature:

			z_all = x     # x=5*20*512*7*7

		else:
			x = x.contiguous().view(self.n_way * (self.n_support + self.n_query), *x.size()[2:])
			z_all = self.feature.forward(x)
			z_all = z_all.view(self.n_way, self.n_support + self.n_query, -1)
		z_support = z_all[:, :self.n_support]   #5*5*512*7*7
		z_query = z_all[:, self.n_support:]   # 5*15*512*7*7
